...
_BASE_SCRIPTS = _REPO_ROOT / "templates" / "base" / "dot_agents" / "scripts"
...
def test_resolver_is_scaffolded_for_base_lifecycle_scripts(preset, variables, tmp_path):
    ...
    resolver = target / ".agents" / "hooks" / "_py.sh"
    ...
    shim = target / ".agents" / "scripts" / "push_branch.sh"
...
def test_json_hook_configs_use_resolver():
    configs = [
        _REPO_ROOT / "templates" / "base" / "dot_agents" / "settings.json.tmpl",
...
def test_dag_workflow_invokes_monitor_via_bash_absolute_path():
    src = (
        _REPO_ROOT / "templates" / "lifecycle" / "dot_agents" / "hooks" / "dag_workflow.py"
    ).read_text()
    ...
    assert '[".agents/scripts/monitor_pr.sh"' not in src
