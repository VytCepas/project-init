...
_REGISTRY_URL_TEMPLATES = {
    ...
    "crates": os.environ.get(
        "PACKAGE_GUARD_CRATES_URL", "https://crates.io/api/v1/crates/{name}"
    ),
}
...
_POPULAR: dict[str, set[str]] = {
    "pypi": {
        ...
        "urllib3", "click", "pyyaml", "setuptools", "pip", "wheel", "certifi",
        "idna", "charset-normalizer", "six", "python-dateutil", "attrs",
        "packaging", "cryptography", "jinja2", "markupsafe", "sqlalchemy",
        "pydantic", "aiohttp", "fastapi", "uvicorn", "scipy", "matplotlib",
        "pillow", "beautifulsoup4", "lxml", "pyjwt", "redis", "celery",
        "gunicorn", "httpx", "typer", "rich",
    },
    "npm": {
        "react", "react-dom", "lodash", "express", "axios", "vue", "webpack",
        "typescript", "eslint", "prettier", "jest", "babel", "chalk", "commander",
        "moment", "dayjs", "uuid", "async", "underscore", "jquery", "next",
        "vite", "rollup", "yargs", "dotenv", "cors", "body-parser", "mongoose",
        "socket.io", "prop-types", "classnames", "redux", "rxjs", "tslib",
        "semver", "glob", "minimist", "debug", "chokidar",
    },
    "crates": {
        "serde", "tokio", "clap", "rand", "regex", "log", "anyhow", "thiserror",
        "reqwest", "async-trait", "futures", "chrono", "uuid", "serde_json",
        "itertools", "once_cell", "lazy_static", "bytes", "hyper", "tracing",
        "clap_derive", "num_cpus", "crossbeam", "parking_lot", "rayon",
        "syn", "quote", "proc-macro2", "libc", "cfg-if", "bitflags",
    },
}
...
def _registry_status(ecosystem: str, name: str) -> int | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "project-init-package-guard"})
