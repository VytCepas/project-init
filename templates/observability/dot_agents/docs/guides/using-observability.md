...
```sh
.agents/scripts/observability.sh report          # text summary + dashboard.html
.agents/scripts/observability.sh report --open    # also open the HTML report
```
...
```sh
.agents/scripts/observability.sh report --session-id <id>
.agents/scripts/observability.sh report --transcript /path/to/session.jsonl
```
...
Delete the `.agents/observability/` directory. The hook self-log goes dormant
