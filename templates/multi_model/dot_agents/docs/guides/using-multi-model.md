...
```bash
cp .agents/multi-model/.env.example .agents/multi-model/.env   # fill in provider keys
.agents/scripts/setup_models.sh
```
...
```bash
.agents/scripts/models.sh list                       # providers/models + pulled Ollama
.agents/scripts/models.sh add ollama qwen3:14b       # ollama pull + register
.agents/scripts/models.sh add deepseek deepseek-v4-pro     # register a cloud model
.agents/scripts/models.sh rm ollama gemma:2b         # ollama rm + unregister
.agents/scripts/models.sh ui                         # open ccr ui (GUI editor)
# tip: alias models="$PWD/.agents/scripts/models.sh"  → then `models list`, etc.
```
