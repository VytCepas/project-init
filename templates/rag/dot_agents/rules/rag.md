...
- **Context lookup order:** `.agents/memory/MEMORY.md` and the vault for curated
...
- **Query it:** `ccc search "where do we validate webhooks"` (semantic) or
  ...
  `ccc mcp` (a stdio server) — see `.agents/docs/guides/using-rag.md`.
- **Not set up yet?** Run `.agents/scripts/setup_rag.sh` (installs `ccc`, pins a
