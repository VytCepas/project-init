...
- RAG is authoritative for **nothing** — a *recall surface* for the fuzzy "where
  ...
  (`.agents/memory/MEMORY.md`, `.agents/docs/adr/`, `.agents/vault/`); it only
...
discover it — set `memory.rag_endpoint` in `.agents/config.yaml`. The index stays
...
Agents follow `.agents/rules/rag.md`: curated facts (memory/vault) → code graph
