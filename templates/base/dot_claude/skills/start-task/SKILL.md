...
4. **Start work** — create the branch, push, open draft PR, arm auto-merge:
   ...
   ```
   This derives the branch name (`<type>/<n>-<slug>`) from the issue title, pushes it, opens a draft PR with the correct `[#n][type]` title and `Closes #n` body, and arms auto-merge if the repo supports it.
