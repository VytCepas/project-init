#!/usr/bin/env bash
# session_end.sh — append a session log to .claude/vault/sessions/.
...
if [ -f "$OPS_LOG" ]; then
  ...
  echo "## [$STAMP] session_end | ${COMMIT_COUNT} commit(s)" >> "$OPS_LOG"
fi

# Run memory lint if available; append warnings to session log
LINT_SCRIPT="$ROOT/.claude/scripts/lint_memory.sh"
