...
# Checks that actually reported. Prefer the PR's rollup: it is the exact set GitHub
# matches the required contexts against.
if [ -n "$PR_NUMBER" ]; then
  REPORTED=$(gh pr view "$PR_NUMBER" --json statusCheckRollup \
    --jq '.statusCheckRollup[]?.name // .statusCheckRollup[]?.context' 2>/dev/null || true)
else
  REPORTED=$(gh api "repos/$REPO/commits/$BRANCH/check-runs" --jq '.check_runs[]?.name' 2>/dev/null || true)
fi
