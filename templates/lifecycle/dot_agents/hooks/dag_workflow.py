...
def _is_upstream_issue_create(cmd: str) -> bool:
    ...
    if not m:
        return False
    # `gh --repo` accepts an optional host ([HOST/]OWNER/REPO), so compare only
    # the trailing owner/repo — otherwise `-R github.com/OWNER/this-repo` parses
    # as `github.com/OWNER`, reads as a different repo, and slips past the wrapper
    # that stamps priority/references onto same-repo issues (PI-882).
