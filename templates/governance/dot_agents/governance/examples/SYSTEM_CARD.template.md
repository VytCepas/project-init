...
<!--
  ...
  To create a real, governed system card:
    1. Copy this file to .agents/governance/SYSTEM_CARD.md
    2. Replace every <REPLACE: ...> placeholder with a real value.
    3. Fill in .agents/governance/ai-declarations.md with the models/APIs you call.
  ...
  not resolve to a filled-in file under .agents/governance/; or if
  `last_reviewed` is older than the staleness window (default 180 days,
  overridable via .agents/governance/config.json — see config.example.json).
