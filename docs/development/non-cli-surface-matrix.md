...
|---|---|---|---|---|---|---|
| **Claude Code VS Code ext** (`anthropic.agents-code`) | **Yes — identical to CLI** (settings.json hooks, skills, CLAUDE.md) | `.agents/skills/` | `.agents/settings.json`, **honored**, PascalCase | shareable project scope = **root `.mcp.json`** (`mcpServers`); `claude mcp add` defaults to a *private* `~/.agents.json` entry | CLAUDE.md / AGENTS.md | HIGH |
...
1. **VS Code agent mode reads `.agents/` directly (camelCase↔PascalCase)?**
   **Confirmed, with nuance.** The Claude Code extension reads `.agents/` with
   full fidelity (hooks honored). Copilot agent mode *also* reads `.agents/`
...
2. **Cursor `hooks.json` event vocabulary + SKILL.md?** **Confirmed.** Cursor
   ...
   `.agents/skills/` for compatibility (skills are free; hooks need translation).
...
`.agents/` (Claude CLI), `.codex/hooks.json` + `.agents/skills/` (Codex),
...
- **Skills are nearly free**: SKILL.md is a de-facto cross-surface standard;
  emitting to `.agents/skills/` + `.agents/skills/` already reaches Claude (ext),
...
- **MCP** has three schemas to target when MCPs are configured: `mcpServers`
  ...
  project MCP — `.agents/` does not carry it.
...
- https://code.agents.com/docs/en/vs-code
- https://code.agents.com/docs/en/hooks
