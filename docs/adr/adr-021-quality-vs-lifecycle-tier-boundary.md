...
| `dot_github/hooks/pre-push` | no-push-to-main + branch-type prefix; git hook (the issue-key check is CI's job, see note) |
...
| fallback/plugin skills `create_issue`, `start_task`, `github_workflow`, `request_review` | drive the lifecycle scripts |
...
`registry-publish.yml.tmpl` (`--delivery library`, ADR-015), `project-init-upgrade.yml.tmpl`
(self-maintenance), `CODEOWNERS.tmpl` (PI-145 governance prompt — GitLab/GitHub both
support it; leave with the existing owner prompt).
