...
```
<your-project>/
├── AGENTS.md          # canonical agent instructions; points into .agents/
...
Principles:
- **One folder, `.agents/`**, for everything agentic. Project root stays clean.
...
|---|---|---|
| **Native** | Everything: deterministic hooks (lifecycle guard, pre-commit gate), skills invoked as `/commands`, settings wiring. Claude Code reads its config from `.claude/`, which the scaffolder projects from the canonical `.agents/` tree — only the config surface Claude reads (settings/skills/commands/agents/rules, each rule's `globs:` rewritten as the `paths:` Claude Code scopes rules by), rebuilt each scaffold/upgrade so it never drifts, plain files so it restores identically on Linux, macOS and Windows. State (memory, ADRs) lives once in `.agents/` | Claude Code (CLI + the Anthropic VS Code extension) |
...
| ↳ *via `--multi-model`* | `CLAUDE.md` | `.claude/skills` | `.claude/settings.json` | root `.mcp.json` | yes |

`AGENTS.md` is the portability backbone (every surface reads it); `CLAUDE.md`
rides alongside for Claude + VS Code. The generated `.agents/CAPABILITIES.md`
...
- Deploy overlay (`--deploy none|cloud-run|fly|k8s|registry|custom`; identity via `--deploy-app` (default: project slug), `--deploy-region` (default: us-central1), `--deploy-health-url` (default: none) — captured interactively in the delivery group, rendered into the descriptor deploy block the orchestrator dispatches on) — opt-in, **service only**. Container targets scaffold a build-once-**by-digest** `deploy.yml` (GitHub Environments: staging auto-deploys, production is **gated**) + a declarative `deploy/environments.yaml`, plus `setup_env_protection.sh` (server-side prod gate, tiered by profile) and `whats_deployed.sh`. `none` (default) = your platform (Vercel/Render/Fly/…) owns deploy.
...
`.agents/docs/guides/secrets.md` covers when and how to escalate to an org
...
`.agents/scripts/setup_github.sh --protect` inside the project to apply
...
- A **SessionStart hook** (`.agents/hooks/session_setup.sh`) syncs
...
`.agents/memory/` and `.agents/vault/` are never compared or touched, and
`.agents/config.yaml` keeps your hand-edited fields (`project_key`, board
...
`.agents/.upgrade-base.json` sidecar. Upgrade re-renders the same preset at the
...
- **Your notes are safe** — `remove memory` unwires the tier but **keeps**
  `.agents/memory/` and `.agents/vault/` (your accumulated notes) by default. To
