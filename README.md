...
- AI governance (`--governance`) — opt-in overlay (ADR-018) that ships **governance-as-code** for projects that build or operate an AI system. Today it scaffolds the **policy layer**: an AI usage policy, approved-tools / data-handling / code-provenance docs, and a NIST RMF crosswalk, adopting NIST AI RMF / ISO 42001 / EU AI Act / OWASP conventions (referenced, not re-authored). A **system card** + **AIBOM** and a **presence-triggered CI gate** land in follow-up increments. Most projects are not AI products — strictly opt-in, off by default. The `governed` preset enables it.
