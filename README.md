...
- **Built-in working skills** — beyond the lifecycle, every scaffold ships on-demand `/command` skills: **`/diagram`** (draw and iterate architecture/code/idea diagrams as committed Mermaid source with live previews), **`/checkpoint`** (save a session handoff to a gitignored file, `/clear`, resume from ~500 tokens), **`/token_efficiency`** (token-frugal working habits), plus `add_hook`, `add_command`, `status`, `review`, and friends. Discover them via `/help` or `.agents/skills/INDEX.md`.
