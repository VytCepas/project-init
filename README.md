...
- AI governance (`--governance`) — opt-in overlay (ADR-018) that ships **governance-as-code** for projects that build or operate an AI system: an AI usage policy, approved-tools / data-handling / code-provenance docs, a NIST RMF crosswalk, a **system card** + **AIBOM** (detected + declared model/MCP inventory), and a **presence-triggered CI gate** that fails on a real system card with missing/invalid fields. Adopts NIST AI RMF / ISO 42001 / EU AI Act / OWASP conventions (referenced, not re-authored). Most projects are not AI products — strictly opt-in, off by default. The `governed` preset enables it.
