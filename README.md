...
- Memory backend (`--memory none|auto|obsidian|obsidian-graphify`) — a **superset ladder** (ADR-024): **none** (vault-free; the `core` preset), **auto** (flat agent-memory files in `.claude/memory/`, no vault — pure files, installs nothing), **Obsidian-only** (auto **plus** a human markdown vault), or **Obsidian + Graphify** (obsidian **plus** a derived code knowledge graph; recommended for code-heavy projects). Each rung adds a retrieval surface without relocating the anchors. The flag overrides the preset's default; the wizard explains each backend and its install cost before asking (#466, #497).
