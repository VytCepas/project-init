...
|---|---|---|
| **Native** | Everything: deterministic hooks (lifecycle guard, pre-commit gate), skills invoked as `/commands`, settings wiring. Claude Code reads its config from `.claude/`, which the scaffolder projects from the canonical `.agents/` tree — only the config surface Claude reads (settings/skills/commands/agents/rules), rebuilt each scaffold/upgrade so it never drifts, plain files so it restores identically on Linux, macOS and Windows. State (memory, ADRs) lives once in `.agents/` | Claude Code (CLI + the Anthropic VS Code extension) |
