...
The corollary bites often: this repo is **not** always a good example of its own output — though the gap keeps closing as the repo adopts what it ships (#637 pip-audit, #638 the Python matrix + `ci-gate`, #639 the `mypy --strict` gate all landed). It still has no `ruff format` gate while the scaffold it ships does (#726). Don't infer template behavior from this repo's config — read `templates/`, or scaffold into a temp dir and look.
...
- **ruff for lint/format, mypy for types** — no black / isort. ruff `check` + `format` own style; `just typecheck` runs `mypy --strict` on `src/` (config in `mypy.ini`), dogfooding the strict typecheck gate the scaffolder ships (#639). Keep annotations precise — strict rejects implicit `Any`.
- **justfile is the command surface** — `just --list` shows the canonical recipes (`setup`, `lint`, `format`, `typecheck`, `test`, `test-quick`, `docs`, `audit`, `ci`); prefer `just <recipe>` over raw tool invocations. Recipes stay thin wrappers — no logic in the justfile.
...
**Bump the plugin version whenever its payload changes (PI-881).** Claude Code caches a plugin by the `version` in its `.claude-plugin/plugin.json`, so a payload edit that forgets to bump serves the stale copy to every existing install — the PI-845 guard fix shipped that way, and #857 patched the same class twice by hand. `plugins/payload-locks.json` now pins each plugin's version to a hash of its shipped payload: `just sync-plugin` **refuses to relock a changed payload under an unchanged version** (bump `plugins/<name>/.claude-plugin/plugin.json` first, then re-run), and `tests/contracts/test_plugin_marketplace.py` + `sync_plugin.py --check` fail CI if the committed lock drifts from the tree. Because plugin payloads are synced from `templates/`, this fires on the template edit that produced the change, not just a direct plugin edit.

**Personal `~/.claude/skills/` copies (PI-681): keep them and `templates/` in step, in the same session.** Some skills the scaffolder ships (e.g. `diagram`) are ALSO maintained as the maintainer's personal `~/.claude/skills/<name>/` copy, which lives outside this repo — no CI can guard it. The rule is bidirectional: an edit made to the personal copy of a shared skill MUST be ported to its `templates/{fallback,lifecycle_fallback}` source (then `uv run python tools/sync_plugin.py` + `just sync-claude`) in the same session, and a template edit should be reflected back into the personal copy — otherwise the two silently diverge (the drift that motivated epic #677). `just skill-drift` (advisory; `tools/skill_drift.py`) prints any drift between the two for the declared shared set; it never gates, because the personal dir isn't versioned here.
