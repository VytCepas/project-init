...
- **Verify the premise before you plan against it** — an issue body, an audit note, or a tool's documented behavior is a *claim*, not a fact. Check it against the code (or run the tool) first. In practice the claim is wrong often enough that this is the cheapest step in the task, not overhead: `@anthropic-ai/claude-code` was never pinned (#689), `.agents/CAPABILITIES.md` does not exist (#686), Go's format gate already worked (#726), and `biome format --check` is not a flag biome accepts (PR #728 review). Say what you verified and how.
- **A test that cannot fail is worse than no test** — after writing a guard, break the thing it guards and watch it fail, then restore. Assertions on a config file's *text* pass while the shell it describes is broken; a filter that skips its own subject reports green forever. This session shipped both bugs before catching them (PR #689, PR #724). State in the PR that you checked.
